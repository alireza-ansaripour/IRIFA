...
class NGO_admin(SummernoteModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if form.is_valid():
            if change:
                ngo = NGO.objects.get(id=obj.id)
                if 'flag' in form.changed_data:
                    ngo.flag.delete(save=False)
            obj.save()
            # image = open(obj.flag.path)
            # size = 200, 170
        ...
        # if form.is_valid():
        #     file = os.path.join(settings.BASE_DIR, 'media/NGO/thumbnails/')
        ...
        #     obj.flag = pic
        #     obj.save()
        #     path = settings.MEDIA_ROOT + '/NGO/flags/' + pic.name
